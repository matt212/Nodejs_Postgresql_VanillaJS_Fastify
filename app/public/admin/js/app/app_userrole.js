...
let baseurlobj = {
  ...
  getpaginateusersearchtypegroupby:
    '/' + currentmuser + '/api/searchtypegroupbyId/'
}
...
let basefunction = function () {
  return {
    ...
    update: function (base) {
      ...
      return this.deleterecord(internpayload)
...
let basemod_modal = {
  ...
  mrolepayloadformat: function (data) {
    ...
    doct.forEach(function (element) {
      if (
        multiselects[element.inputtextval] != undefined &&
        Object.keys(multiselects).length != 0
      ) {
        interns[element.inputtextval] =
          multiselects[element.inputtextval][0][element.inputtextval]
      } else {
    ...
    var isactiveparam = data.datapayload.recordstate
    ...
    o.payset = providedre
    o.delObj = {
      roleid: internrolename[0].roleid
    }
    base.datapayload = o
    return base
  },
    ...
    formatresponse.forEach2(function (data) {
      if (data.inputtype == 'textbox') {
        ...
        var coremulti = interncontent.map2(function (da) {
          var y =
            da[data.inputtextval].indexOf(',') != -1
              ? da[data.inputtextval].split(',')
              : da[data.inputtextval]
  ...
  formatresponse: function (data) {
    ...
    var result = equijoin(
      ...
      'key',
      'inputtextval',
      ({ vals }, { inputtype, inputtextval, inputname }) => ({
        inputtype,
        inputtextval,
        inputname,
        vals
...
let basemultiselectaccess = {
  ...
  remotefuncusername: function (data) {
    return new Promise(function (resolve, reject) {
      ...
      basefunction()
        .getpaginateusersearchtypegroupby(internbase)
        .then(function (argument) {
          var sets = argument.rows
          if (sets[0] != undefined) {
            var internfield = Object.keys(sets[0])
