...
/*
 * Initialize Socket.IO only when upload/download
 * functionality actually needs it.
 */
function initSocket() {
  ...
  socket.on('newsdownloadsets', function (data) {
    ...
    var ids = setInterval(function () {
      ...
      if (loop === 101) {
        ...
        $('#lbluploadprogress' + filenome).html(
          loop + '%'
        )

        $('#dvuploadprogress' + filenome).css(
          'width',
          loop + '%'
        )
  ...
  socket.on('newsdownloadsetprogressstats', function (data) {
    ...
    $('#libasenotificationrow').append(
      '<span style="color: #28669c;" id="spn_dv' +
        filenames +
        '">Data processing in progress !</span>' +

      '<div style="margin:0 0 0 15px" id="dv_set_progress' +
        filenames +
        '">' +

      '<small class="pull-right">' +
        '<label id="lbluploadprogress' +
        filenames +
        '">%</label>' +
      '</small>' +
      ...
      '<div class="ripple ripple-on ripple-out" ' +
        'style="left: 205.203px; top: 42px; background-color: rgb(33, 150, 243); transform: scale(32.875);">' +
