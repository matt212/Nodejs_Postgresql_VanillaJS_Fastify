let datatransformutils = {
      ...
      if (r1[0].childcontent != undefined) {
        ...
        if (Array.isArray(r2) && r2.length) {
          r2.forEach(function(dt) {
            redlime.push(r1[0].childcontent.filter((dt1) => dt1.val == dt)[0].text);
          });
          return redlime.join(",");
        } else {
  ...
  getCartesian: function(object) {
    return Object.entries(object).reduce(
      (r, [k, v]) => {
        var temp = [];
        r.forEach((s) => (Array.isArray(v) ? v : [v]).forEach((w) => (w && typeof w === "object" ? datatransformutils.getCartesian(w) : [w]).forEach((x) => temp.push(Object.assign({}, s, {
          [k]: x
        })))));
        return temp;
      },
  ...
  flat: (v) => v && typeof v === "object" ? Object.values(v).flatMap(datatransformutils.flat) : v,
  ...
  editMultiSelect: function(obj) {
    ...
    var res = obj.content.map(function(data) {
      return a1.map(function(da) {
        ...
        if (data[da.inputCustomMapping].indexOf(",") != -1) {
          ...
          /**end region */
          inten[da.inputtextval] = data[da.inputCustomMapping].split(",").map(function(dt, i) {
            return {
              key: da.inputtextval,
              text: da.childcontent != undefined ? datatransformutils.staticValsMapping(da.inputCustomMapping, dt) : dt,
              vals: isNaN(parseInt(data[da.inputname].split(",")[i])) ? data[da.inputname].split(",")[i] : parseInt(data[da.inputname].split(",")[i]),
            };
          });
        } else {
          ...
          inten[da.inputtextval] = {
            key: da.inputtextval,
            text: da.childcontent != undefined ? datatransformutils.staticValsMapping(da.inputCustomMapping, data[da.inputCustomMapping]) : data[da.inputCustomMapping],
            vals: isNaN(parseInt(data[da.inputname])) ? data[da.inputname] : parseInt(data[da.inputname]),
            //vals: y
          };
        }
        return inten;
    ...
    let basegen = datatransformutils.generateMappedValues(a1, obj.content[0])
    console.log(basegen)
    // basegen.forEach(function(dt) {
    ...
    //       val: dt.vals[index]
    //     })
    //     obj.multiselectfunc[dt.controlname].onsearchtext({
    //       key: dt.controlname,
    //       text: text,
    //       val: dt.vals[index]
    //     });
  ...
  generateMappedValues: function(config, data) {
    return config.filter(({
      inputtextval
    }) => inputtextval !== "recordstate").map(({
      inputname,
      inputCustomMapping,
      inputtextval
    }) => {
      const mappingKey = inputCustomMapping || inputtextval;
      const map = data[`${mappingKey}_map`];
      if (map && typeof map === "object" && !Array.isArray(map)) {
        const mappedValues = Object.values(map);
        return {
          key: mappingKey,
          controlname: inputtextval,
          text: mappedValues.map(item => item.value),
          vals: mappedValues.map(item => item.key)
        };
      }
      return {
        key: mappingKey,
        controlname: inputtextval,
        text: datatransformutils.toArray(data[mappingKey]),
        vals: datatransformutils.toArray(data[inputname])
      };
    });
  },
  toValueArray: function(value) {
    return datatransformutils.toArray(value).map(item => !isNaN(item) ? Number(item) : item);
  },
...
const doChunk = (list, size) => list.reduce(
  (r, v) => (!r.length || r[r.length - 1].length === size ? r.push([v]) : r[r.length - 1].push(v)) && r,
  []);
