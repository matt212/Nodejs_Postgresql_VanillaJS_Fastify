...
let baseloadsegments = {
  initialdatatableload: function() {
    ...
    if (!base.searchdatefilter ||(typeof base.searchdatefilter === "object" &&
    Object.keys(base.searchdatefilter).length === 0)) {
      const end = new Date();
      const start = new Date();
      start.setDate(start.getDate() - 7);
      const formatDate = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
      base.searchdatefilter = {
  ...
  getapptoken: function(ajaxbase) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      ajaxutils.basepostmethod(ajaxbase).then(baseloadsegments.setconfig).then(function(argument) {
        resolve(argument)
  ...
  populateaxiskeys: function() {
    ...
    console.log("asasasasasasassasa")
    console.log(jsonintern)
