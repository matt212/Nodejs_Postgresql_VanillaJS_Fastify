...
async function baseDecorator (fastify, options) {
  ...
  const loginRbacCache = new Map()
const loginRbacInFlight = new Map()
const LOGIN_RBAC_CACHE_TTL = 5 * 60 * 1000
//const LOGIN_RBAC_CACHE_TTL = 15 * 60 * 1000
async function getCachedRBAC(userId) {

  const cached = loginRbacCache.get(userId)

  if (cached && cached.expires > Date.now()) {
    return cached.value
  }

  if (loginRbacInFlight.has(userId)) {
    return await loginRbacInFlight.get(userId)
  }

  const promise = rbac(userId)

  loginRbacInFlight.set(userId, promise)
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      loginRbacCache.set(userId, {
        value: data,
        expires: Date.now() + LOGIN_RBAC_CACHE_TTL
      })
    }
    ...
    loginRbacInFlight.delete(userId)
  }
}
  /*
...
const trustedRBAC = await getCachedRBAC(userId)
...
const userId = user.dataValues.muserid

const data = await getCachedRBAC(userId)
