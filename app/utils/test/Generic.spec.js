...
let controlPayset = function(validationConfig, entry) {
  ...
  if (validationConfig.validationmap[0].hasOwnProperty('inputtextval')) {
    fieldtype = validationConfig.validationmap.filter(o1 => o1.inputtextval == entry)[0]
  } else {
    fieldtype = validationConfig.validationmap.filter(o1 => o1.inputname == entry)[0]
  }
  return fieldtype
}
let controlPaysetMulti = function(validationConfig, entry) {
  let fieldtype = ''
  fieldtype = validationConfig.validationmap.filter(o1 => o1.inputCustomMapping == entry)[0]
  return fieldtype
}
...
let customMultiInsertDelete = function(testbase, evalModulename) {
  ...
  o.multiInsertforSearch = function(val) {
    ...
    return new Promise((resolve, reject) => {
      Promises.mapSeries(testbase.schemaBaseValidatorPayloadAr, o.multiInsert).then(a => {
        var o1 = {
...
let massDelete = function(testbase) {
  return (promise = new Promise((resolve, reject) => {
    Promises.mapSeries(testbase.Deletesampledatset, baseDataCleanUp).then(function(a) {
      resolve(a)
    })
  }))
}
let metaTestcaseGen = function(a) {
  ...
  let validationConfig = require('../../routes/utils/' + testbase.evalModulename + '/validationConfig.js')
...
let referentialCustomStack = function(s1, l1) {
  return (promise = new Promise((resolve, reject) => {
    customRefentialModnameInsert(s1).then(function(dt) {
      //console.log(dt)
      //done()
      var inter1 = []
      var inter2 = []
      getParentfromValidationMap(l1.b).forEach(function(val) {
        ...
        if (lime1.filter(e => e[p1.a1] != undefined).length > 0 && lime2.filter(e => e[p1.a2] != undefined).length > 0) {
          inter1.push({
            a: lime1,
            b: lime2
          })
...
let customTestsInit = function(testbase, validationConfig) {
  ...
  testbase.schemaValValidatorPayload = schemaValueValidatorPayload(appField, testbase.schemaBaseValidatorPayload)
  /*Schema validatior with blank fields one by ony fields*/
  testbase.schemaValValidatorPayloadBlank = schemaValueValidatorPayloadBlank(appField, testbase.schemaBaseValidatorPayload)
  /*Schema validatior with Max  fields validations one by ony fields*/
  testbase.schemaValValidatorPayloadMaxLenght = schemaValValidatorPayloadMaxLenght(appField, testbase.schemaBaseValidatorPayload)
  return testbase
}
let getParentfromValidationMap = function(validationConfig) {
  let a1 = validationConfig.validationmap.map(a => (a.inputParent != undefined ? a.inputParent : undefined)).filter(Boolean)
  return a1
}
let getIDfromValidationMap = function(validationConfig) {
  let a1 = validationConfig.validationmap.filter(a => a.inputname != 'recordstate').map(a => (a.inputname != undefined ? a.inputname : undefined))
  return a1
}
let getContentFieldsfromValidationMap = function(validationConfig, val) {
  let a1 = validationConfig.validationmap.filter(function(dt) {
  ...
  return (promise = new Promise((resolve, reject) => {
    try {
      ...
      ModularizeDataGen(a1).then(function(data) {
        let a = initMultiPayloadforSearch(testbase.evalModulename, data, validationConfig.validationmap, a1)
        ...
        let o = validationConfig.validationmap.filter(a => (a.inputParent == undefined)).map(task => task.inputname)
        //end
        ...
        // })
        var fr2 = testbase.schemaBaseValidatorPayloadAr.map((user, i) => {
          var updatedUser = datatransformutils.updateJSONByKEY(user, data[0].multiControlDataSetAr[i].evalModulename + 'id', data[0].multiControlDataSetAr[i].createdID)
          return {
            ...updatedUser,
            modnameid: a.singleDataSet[0].modnameid
          }
        })
        //end Here
...
let loginsuccess = function(cred) {
  try {
    return (promise = new Promise((resolve, reject) => {
      var user = cred
      server.post('/login').send(user).expect('Content-type', /json/).expect(200).end(function(err, res) {
        if (err) {
          resolve({
            status: 'fail',
            content: err
...
let ModularizeDataGen = function(applyfields) {
  return (promise = new Promise((resolve, reject) => {
    Promises.mapSeries(applyfields, ModularizeDataGen1).then(a => {
      resolve(a)
    }).catch(err => console.log(err))
  }))
}
...
let customRefentialModnameInsert = function(modulename) {
  return new Promise((resolve, reject) => {
    ...
    MultiControlTestCaseGen(l1.a, l1.b).then(function(data) {
...
let genericApiPost = function(data) {
  return (promise = new Promise((resolve, reject) => {
    try {
      server.post(data.apiUrl).send(data.payload).set('x-access-token', data.token).expect('Content-type', /json/).expect(data.responseCode).end(function(err, res) {
        if (err) {
...
let schemaValValidatorPayloadMaxLenght = function(baseapplyFields, baseObj) {
  var interimAr = []
  baseapplyFields.forEach(function(entry) {
    var o = datatransformutils.updateJSONByKEY(baseObj, entry, createModulePayLoad.makeMaxlenghtpayload(baseObj, entry))
    interimAr.push({
      key: entry,
      schemaContent: o
    })
...
let initMultiPayloadforSearch = function(dt, data, validationmap, ap) {
  ...
  let o = {
    ...
    insertUpdateDelete: ar2[0].map((item, i) => Object.assign({}, item, ar2[1] != undefined ? ar2[1][i] : ar1[0][i])),
  ...
  let o1 = {
    searchtype: o.searchtype.map((item, i) => Object.assign({}, item, dt == 'mrole' ? internAccesstypes[i] : [])),
    insertUpdateDelete: o.insertUpdateDelete.map((item, i) => Object.assign({}, item, dt == 'mrole' ? internAccesstypes[i] : [])),
    singleDataSet: o.singleDataSet.map((item, i) => Object.assign({}, item, dt == 'mrole' ? internAccesstypes[i] : [])),
    baseData: data
  }
  return o1
}
...
let PrimarytestInit = function(testbase) {
  return new Promise((resolve, reject) => {
    FirstTimeloadCurrentModule().then(loginsuccess).then(loadCurrentModule).then(function(data) {
      testbase.token = data
      console.log('****************login and loaded the module  successfully****************')
      customMultiInsertDelete(testbase, testbase.evalModulename).multiInsert(testbase.schemaBaseValidatorPayload).then(customMultiInsertDelete(testbase, testbase.evalModulename).multiInsertforSearch).then(function(data) {
        console.log('*****Multi Records are inserted sucessfully*****')
...
let baseDataCleanUp = function(dt) {
  return new Promise((resolve, reject) => {
    ...
    customMultiInsertDelete(testbase, testbase.evalModulename).multiDelete(testbase.DelAr).then(function(data) {
      resolve('success')
    })
  })
}
...
let consolidatedPayload = function() {
  ...
  o.payload5 = function(testbase, evalModulename) {
    testbase.apiUrl = '/' + evalModulename + dep.update
    testbase.responseCode = 400
    testbase.schemaBaseValidatorPayload[testbase.evalModulename + 'id'] = undefined
    testbase.payload = testbase.schemaBaseValidatorPayload
    return testbase
  }
  o.payload6 = function(testbase, evalModulename) {
    testbase.apiUrl = '/' + evalModulename + dep.update
    testbase.responseCode = 200
    testbase.schemaBaseValidatorPayload[testbase.evalModulename + 'id'] = testbase.InsertID
    testbase.payload = testbase.schemaBaseValidatorPayload
    return testbase
  }
  ...
  (o.payload17 = function(testbase, entry, evalModulename, validationConfig) {
    testbase.apiUrl = '/' + evalModulename + dep.searchtype[1]
    testbase.responseCode = 200
    ...
    if (fieldtype.fieldtypename == 'DATE') {
      o1.daterange = {
        startdate: new Date(testbase.schemaBaseValidatorPayload[entry]).toLocaleDateString(),
        enddate: new Date(testbase.schemaBaseValidatorPayload[entry]).toLocaleDateString()
      }
      ...
      if (fieldtype.fieldtypename.toLowerCase() != fieldtype.fieldvalidatename) {
  ...
  (o.payload18 = function(testbase, entry, evalModulename, validationConfig) {
    testbase.apiUrl = '/' + evalModulename + dep.searchtype[1]
    testbase.responseCode = 200
    var o1 = JSON.parse(JSON.stringify(loadModulePayLoad))
    let fieldtype = controlPayset(validationConfig, entry)
    ...
    if (fieldtype.fieldtypename == 'DATE') {
      o1.daterange = {
        startdate: new Date(testbase.schemaBaseValidatorPayloadAr[0][entry]).toLocaleDateString(),
        enddate: new Date(testbase.schemaBaseValidatorPayloadAr[0][entry]).toLocaleDateString()
      }
      o1.datecolsearch = entry
      o1.disableDate = false
    } else if (fieldtype.fieldtypename == 'boolean' || fieldtype.fieldtypename == 'BIGINT' || fieldtype.fieldtypename == 'INTEGER') {
  ...
  (o.payload19 = function(testbase, entry, evalModulename, validationConfig) {
    ...
    if (fieldtype == 'DATE') {
      ...
      o1.searchparam = multicolumngenAr(testbase.schemaBaseValidatorPayloadAr[0], entry)
      // o1.disableDate = true
      o1.daterange = {
        startdate: new Date().toLocaleDateString(),
        enddate: new Date().toLocaleDateString()
  ...
  (o.payload20 = function(testbase, evalModulename, validationConfig) {
    ...
    var searchVal = testbase.schemaBaseValidatorPayload[Object.keys(testbase.schemaBaseValidatorPayload)[0]]
  ...
  o.payload23 = function(testbase, entry, evalModulename, validationConfig) {
    ...
    if (fieldtype == 'DATE') {
      o1.daterange = {
        [undefined]: new Date(testbase.schemaBaseValidatorPayload[entry]).toLocaleDateString(),
        [undefined]: new Date(testbase.schemaBaseValidatorPayload[entry]).toLocaleDateString()
      }
      o1.datecolsearch = entry
      o1.disableDate = false
  ...
  o.payload191 = function(testbase, entry, evalModulename, validationConfig, schemaBaseValidatorPayloadAr1) {
    testbase.apiUrl = '/' + evalModulename + dep.searchtype[1]
    testbase.responseCode = 200
    var o1 = JSON.parse(JSON.stringify(loadModulePayLoad))
    ...
    if (fieldtype == 'DATE') {
      o1.daterange = {
        startdate: new Date(schemaBaseValidatorPayloadAr1[0][entry]).toLocaleDateString(),
        enddate: new Date(schemaBaseValidatorPayloadAr1[0][entry]).toLocaleDateString()
      }
      o1.datecolsearch = entry
      o1.disableDate = false
    } else if (fieldtype == 'boolean') {
  ...
  o.payload171 = function(testbase, entry, evalModulename, validationConfig, schemaBaseValidatorPayload1) {
    testbase.apiUrl = '/' + evalModulename + dep.searchtype[1]
    testbase.responseCode = 200
    var o1 = JSON.parse(JSON.stringify(loadModulePayLoad))
    let fieldtype = controlPayset(validationConfig, entry)
    if (fieldtype.fieldtypename == 'DATE') {
      o1.daterange = {
        startdate: new Date(schemaBaseValidatorPayload1[entry]).toLocaleDateString(),
        enddate: new Date(schemaBaseValidatorPayload1[entry]).toLocaleDateString()
      }
      o1.datecolsearch = entry
      o1.disableDate = false
    } else if (fieldtype.fieldtypename == 'boolean' || fieldtype.fieldtypename == 'BIGINT' || fieldtype.fieldtypename == 'INTEGER') {
  ...
  o.payload181 = function(testbase, entry, evalModulename, validationConfig, schemaBaseValidatorPayload1) {
    testbase.apiUrl = '/' + evalModulename + dep.searchtype[1]
    testbase.responseCode = 200
    var o1 = JSON.parse(JSON.stringify(loadModulePayLoad))
    ...
    if (fieldtype.fieldtypename == 'DATE') {
      o1.daterange = {
        startdate: new Date(schemaBaseValidatorPayload1[0][entry]).toLocaleDateString(),
        enddate: new Date(schemaBaseValidatorPayload1[0][entry]).toLocaleDateString()
      }
      o1.datecolsearch = entry
      o1.disableDate = false
    } else if (fieldtype.fieldtypename == 'boolean' || fieldtype.fieldtypename == 'BIGINT' || fieldtype.fieldtypename == 'INTEGER') {
  ...
  o.payload201 = function(testbase, evalModulename, validationConfig, {
