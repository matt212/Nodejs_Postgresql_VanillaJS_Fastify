...
// ============================================================
// EMPLOYEES - CENTRAL LOCATOR / CONTROL / API REGISTRY
...
const EMPLOYEES = {
    ...
    api: {
        ...
        count: "/employees/api/searchtypeCount/",
        update: "/employees/api/update/",
        create: "/employees/api/create/",
...
async function openControlBar(page) {
    ...
    await page.locator(
        EMPLOYEES.locators.controlBarCollapse
    ).click();

    await expect(
        page.locator(EMPLOYEES.locators.dateRange)
    ).toBeVisible();
}
...
async function applyDateRange(page) {
    ...
    // --------------------------------------------------------
    // Open / activate the daterangepicker
    // --------------------------------------------------------
0
    await page.locator(
        EMPLOYEES.locators.dateRange
    ).click();
    ...
    console.log(
        'Date range:',
        await page
            .locator(EMPLOYEES.locators.dateRange)
            .inputValue()
    );


    // --------------------------------------------------------
    // Register BOTH API listeners BEFORE clicking Apply
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // Wait for both backend responses
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // Wait for actual report rendering
    // --------------------------------------------------------

    await expect(
        page.locator(
            EMPLOYEES.locators.reportContainer
        )
    ).not.toHaveClass(
    ...
    await expect(
        page.locator(
            EMPLOYEES.locators.reportContent
        ).first()
    ).toBeVisible();

    await expect(
        page.locator(
            EMPLOYEES.locators.table
        )
    ).toBeVisible();
...
async function openFilterBar(page) {
            ...
            await page.locator(
                EMPLOYEES.locators.filterBarParentToggle
            ).click();
...
async function getFirstRowData(page) {
            ...
            const headers = page.locator(
                EMPLOYEES.locators.tableHeaders
            );
...
test(
    ...
    async ({ page }) => {
        ...
        await expect(
            page.locator(EMPLOYEES.locators.reportContent).first()
        ).toBeVisible();
    }
);
...
test(
    ...
    async ({ page }) => {
        ...
        await expect(
            page.locator(EMPLOYEES.locators.table)
        ).toBeVisible();

        await expect(
            page.locator(EMPLOYEES.locators.tableRows).first()
        ).toBeVisible();
    }
);
...
test(
    ...
    async ({ page }) => {
        ...
        await expect(page.locator(EMPLOYEES.locators.totalUsers)).toHaveText(/\d+/);
    }
);
...
test(
    ...
    async ({ page }) => {
        ...
        await page.locator(
            EMPLOYEES.locators.consolidatedSearch
        ).fill(searchValue);


        await page.locator(
            EMPLOYEES.locators.consolidatedSearchAction
        ).click();
...
test(
    ...
    async ({ page }) => {
        ...
        await page.locator(
            EMPLOYEES.locators.dynamicFilterToggle
        ).click();


        await expect(
            page.locator(EMPLOYEES.locators.dynamicFilterContainer)
        ).toBeVisible();
...
test(
    ...
    async ({ page }) => {
        ...
        const inputs =
            page.locator(
                EMPLOYEES.locators.dynamicFilterInputs
            );
        ...
        for (
            ...
            await expect(
                page.locator(EMPLOYEES.locators.totalUsers)
            ).not.toHaveText('0');
...
test('16 - CRUD - Create Employee using validationmap', async ({ page }) => {
    ...
    await page.locator(
        EMPLOYEES.locators.createEmployee
    ).click();
    ...
    const recordStateInput =
        page.locator(EMPLOYEES.locators.recordStateInput);

    const recordStateControl =
        page.locator(
            EMPLOYEES.locators.recordStateControl
        );
    ...
    const submitButton =
        page.locator(EMPLOYEES.locators.modalSubmit);
    ...
    const [
        ...
        page.waitForResponse(
            r =>
                r.url().includes(EMPLOYEES.api.search) &&
                r.status() >= 200 &&
                r.status() < 300
        ),
    ...
    const createdRow =
        page.locator(EMPLOYEES.locators.tableRows)
            .filter({
                hasText: anchorValue
            })
            .first();
...
test('18 - CRUD - Create and Update Employee using validationmap', async ({ page }) => {
 ...
 await page.waitForSelector(EMPLOYEES.locators.tableRows);
    employeeRow =
        page.locator(EMPLOYEES.locators.tableRows)
            .filter({
...
test('19 - CRUD - Update each field individually using validationmap', async ({ page }) => {
    ...
    await expect(page.locator(EMPLOYEES.locators.table)).toBeVisible();
...
test(
    ...
    async ({ page }) => {
        ...
        await page.locator(
            EMPLOYEES.locators.modalSubmit
        ).click();
        ...
        await expect(
            page.locator(EMPLOYEES.locators.reportContent)
        ).toBeVisible({
            timeout: 30000
        });
   ...
   const pagingMenu =
    page.locator(EMPLOYEES.locators.pagingMenu);
...
async function selectOneDynamicEmployeeFilter(page) {
    ...
    await expect(page.locator(EMPLOYEES.locators.dynamicFilterContainer))
...
async function applyDynamicEmployeeFilter(page) {
    ...
    await expect(
        page.locator(EMPLOYEES.locators.reportContainer)
    ).not.toHaveClass(
        /loading-report-container/,
        {
            timeout: 30000
...
test(
    ...
    async ({ page }) => {
        ...
        const pageSizeInput = page.locator(EMPLOYEES.locators.pageSize);
        const rows = page.locator(EMPLOYEES.locators.tableRows);
        ...
        const pageSizeResponsePromise = page.waitForResponse(
            response =>
                response.url().includes(EMPLOYEES.api.search) &&
                response.request().method() === 'POST' &&
                response.status() === 200,
            { timeout: 30000 }
        );
        ...
        const totalUsers = Number(
            (await page.locator(EMPLOYEES.locators.totalUsers).textContent() || '0').trim()
        );
...
test(
    ...
    async ({ page }) => {
        ...
        const originalTotal = Number(
            (
                await page.locator(EMPLOYEES.locators.totalUsers).textContent()
                || '0'
            ).trim()
        );
        ...
        if (expectedRestoredTotal > 0) {
            ...
            const rowCount =
                await page.locator(EMPLOYEES.locators.tableRows).count();
...
test(
    ...
    async ({ page }) => {
        ...
        const pageSizeInput =
            page.locator(EMPLOYEES.locators.pageSize);
...
test(
    ...
    async ({ page }) => {
        ...
        await page.locator(EMPLOYEES.locators.modalSubmit).click();
        ...
        const deletedOption =
            page.locator(EMPLOYEES.locators.deleted);
        ...
        const consolidatedSearch =
            page.locator(EMPLOYEES.locators.consolidatedSearch);
        ...
        const newestOption =
            page.locator(EMPLOYEES.locators.newest);
